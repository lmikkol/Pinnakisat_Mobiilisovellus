...
loginRouter.post('/', async (request, response) => {
  ...
  response
    .status(200)
    .send({ token, name: user.email, userRole: user.role, id: user._id, contests: user.contests, sightings:user.sightings, firstName: user.firstName, lastName: user.lastName })
})
